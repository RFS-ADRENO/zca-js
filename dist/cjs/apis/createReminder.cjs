...
const createReminderFactory = utils.apiFactory()((api, ctx, utils$1) => {
    const serviceURL = {
        [Enum.ThreadType.User]: utils$1.makeURL(`${api.zpwServiceMap.group_board[0]}/api/board/oneone/create`),
        [Enum.ThreadType.Group]: utils$1.makeURL(`${api.zpwServiceMap.group_board[0]}/api/board/topic/createv2`),
    };
    /**
    ...
    return async function createReminder(options, threadId, type = Enum.ThreadType.User) {
        ...
        const params = type === Enum.ThreadType.User
            ? {
                toUid: threadId,
                type: 0,
                color: options.color && options.color.trim() ? utils.hexToNegativeColor(options.color) : -16245706,
