...
export function blockUserFactory(serviceURL: string) {
    return async function blockUser(userId: string) {
        ...
        const result = await handleZaloResponse<BlockUserResponse>(response);
        if (result.error) throw new ZaloApiError(result.error.message, result.error.code);

        return result.data as BlockUserResponse;
    };
}
